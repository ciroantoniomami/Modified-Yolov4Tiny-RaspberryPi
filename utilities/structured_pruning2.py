...
def prune_model(model):
    ...
    def prune_conv(conv, amount=0.5):
    ...
    for m in model.modules():
        ...
        #    prune_conv( m.conv1.conv)
        #    prune_conv( m.conv2.conv)
        #    prune_conv( m.conv3.conv)
        #    prune_conv( m.convaux.conv)
...
if __name__ == '__main__':
    ...
    test_loader = test_loader.to("cuda:0")
    S=[13, 26]
    ...
    model.load_state_dict(torch.load('models/model.pt',map_location = use_gpu_if_possible()))
    ...
    with open('qp01bi.tflite', 'wb') as f:
